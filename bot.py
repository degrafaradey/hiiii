...
async def handle_message(message: Message):
    ...
    # (админу самому пересылать не нужно, если он пишет не в рамках квиза)
    if progress is None or not progress.active:
        logging.info("DEBUG branch: no active quiz progress for this user")
        if user_id == ADMIN_ID:
            return  # админ не в квизе -> просто игнорируем его сообщения здесь
        if ADMIN_ID:
